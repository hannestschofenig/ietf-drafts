...
\begin{description}
\item[MD5] Collisions can be easily found [Wang et al.][Klima06]``Target'' collisions
can be found with $2^{52}$ effort [Stevens et al.]. 
...
\item[HMAC] Theoretical forgery attacks [Kim et al.]
\end{description}
\vspace{.05 in}
...
\begin{slide}
\heading{The Wang collision attack}
...
\begin{slide}
\heading{The Stevens target collision attack}
...
if(A)  }\\
\multicolumn{2}{|l|}{
\hspace{.25 in}display X} \\
\multicolumn{2}{|l|}{
else} \\
\multicolumn{2}{|l|}{
\hspace{.25 in}display Y}\\
\hline
\end{tabular}
\end{itemize}
...
\heading{Targeted Collisions and Certificates}

\begin{itemize}
\item Randomized values make regular collision attacks impractical
\begin{itemize}
\item Cert is packed too tightly to tweak DNs
\item Best attack is to demonstrate two certs with same DN and different public keys
\end{itemize}
...
\end{itemize}
\item This isn't really practical
\begin{itemize}
...
\begin{slide}
\heading{Controllable Collisions and certificates}
...
\end{itemize}
\end{itemize}
\item Attacker is likely to need to try the attack a number of
times
...
\begin{itemize}
\item Attacker should be able to forge a cert of his 
...
\item Signatures
\end{itemize}
\item Only signatures are threatened
...
\begin{itemize}
\item But these don't apply if the interior hash function is weak
\end{itemize}
...
\begin{itemize}
\item Given $X, HMAC(K, X)$, extract $K$
\item $|K|$ must be not too much larger than length of HMAC output
\end{itemize}
\item Plausible scenarios
\begin{itemize}
\item IPsec (remember, EtA)
\begin{itemize}
\item Extract $K_m$, work backward to master key (typically DH ZZ)
\begin{itemize}
\item Remember: use $H(K)$ when $|K| > blocksize$, 
\end{itemize}
\end{itemize}
\item SSL/TLS (only in NULL encryption mode)
\begin{itemize}
\item Extract $K_m$, work backward to master secret
\begin{itemize}
\item This doesn't work with RSA ($|MS|$ = 384 bits)
\item Plausible with DH but need to work backward to PMS
\end{itemize}
\end{itemize}
\end{itemize}
...
\begin{slide}
\heading{Per-record MAC}

\begin{itemize}
\item Not much to worry about here
\begin{itemize}
\item HMAC doesn't depend on collision-resistance
\item Not clear about how much preimage-resistance is required
\end{itemize}
...
\begin{itemize}
\item Mostly the same issues as SSL/TLS
\begin{itemize}
\item Which certificates to use if you have more than one?
\item Which hash functions initiator should use in ``revised public key mode''
\end{itemize}
